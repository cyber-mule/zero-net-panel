...
func seedNodes(tx *gorm.DB) error {
	...
	laNode := repository.Node{
		...
		Country:           "US",
		ISP:               "NTT",
		Status:            status.NodeStatusMaintenance,
	...
	bindings := []repository.ProtocolBinding{
		{
			...
			Listen:          "0.0.0.0:443",
			AccessPort:      443,
			Status:          status.ProtocolBindingStatusActive,
			KernelID:        "edge-hk-1-vless",
			SyncStatus:      status.ProtocolBindingSyncStatusSynced,
			HealthStatus:    status.ProtocolBindingHealthStatusHealthy,
			Tags:            []string{"edge", "hk"},
			Description:     "香港入口示例",
		...
		},
		{
			...
			SyncStatus:      status.ProtocolBindingSyncStatusSynced,
			HealthStatus:    status.ProtocolBindingHealthStatusDegraded,
			Tags:            []string{"edge", "us"},
			Description:     "洛杉矶入口示例",
...
func seedProtocolEntries(tx *gorm.DB) error {
	...
	for _, binding := range bindings {
		...
		entries = append(entries, repository.ProtocolEntry{
			...
			Status:       status.ProtocolEntryStatusActive,
