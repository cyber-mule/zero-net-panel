...
1. **注册迁移**：发布版已将历史迁移重建为初始化基线（版本 `2026010501`）。后续 Schema 变更需在 `internal/bootstrap/migrations/registry.go` 继续新增 `Migration`，并使用递增的时间戳作为版本号；若从旧的预发布库升级，建议备份后重新初始化数据库。
...
| 同上 | `500101` | 内核握手失败 | 验证节点 `control_endpoint`、认证信息与网络连通性；可使用 `curl`/`grpcurl` 手动探测。 |
...
- **风险提示**：回滚会删除新建表或字段，可能导致最新数据不可恢复。确认业务已停机或无写入流量，避免在回滚过程中产生新的业务数据。当前发布版仅有初始化基线迁移，回滚意味着清空全部表结构，应仅在测试或重新初始化时使用。
