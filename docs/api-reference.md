...
> 时间字段统一为 Unix 秒（UTC）。
> 字段必填/可选由 go-zero 的请求结构体标签决定：带有 `,optional` 标记的字段为可选，其余为必填。
> 若缺少必填字段或类型不匹配，`httpx.Parse` 会返回 `400`，错误消息形如 `invalid request: ...`；业务校验失败会返回更具体的 `message` 提示。
...
- 业务错误返回：`{"message": "..."}`，并带有对应 HTTP 状态码。
  - 请求解析错误返回：`{"message": "invalid request: ..."}`
  - 常见状态码：
    - `400` 参数非法
    - `401` 未登录或令牌失效
    - `403` 权限不足或访问受限
...
#### GET /api/v1/{adminPrefix}/protocols

- 说明：返回已配置的协议列表（用于前端下拉选项，避免硬编码）
  - 响应：
    - `protocols` []string
