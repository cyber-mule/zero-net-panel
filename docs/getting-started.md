...
| 生产环境 | 基于 `etc/znp-api.yaml` 衍生 | MySQL 或 PostgreSQL | 调整 `Auth` 与 `Credentials` 密钥、缓存 Provider 及节点 `control_endpoint`，启用 HTTPS/TLS 与只读副本。 |

将配置副本放置在安全位置（如 `/etc/znp/production.yaml`），并通过 `--config`/`-f` 参数显式指定。

前后端分离部署时，请在配置中设置 `CORS` 允许前端域名访问。
